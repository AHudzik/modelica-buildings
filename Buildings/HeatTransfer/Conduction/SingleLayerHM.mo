...
model SingleLayerHM
  ...
  Buildings.HeatTransfer.Conduction.BaseClasses.VapourDiffusionCoefficient kd[
    nSta + 1];

  Modelica.SIunits.Pressure pw[nSta] "Vapour Pressure at the state";

  Buildings.HeatTransfer.Conduction.BaseClasses.RelativeHumidity phi[nSta](
    each start=phi_ini,
    max=1.0,
    min=0.0) "Relative humidity";

  Buildings.HeatTransfer.Conduction.BaseClasses.MoistureStorageCapacity dw_dphi[
  ...
  Buildings.HeatTransfer.Conduction.BaseClasses.LiquidConductionCoefficient
...
equation
  ...
  for i in 2:nSta loop
    UA[i] = 1.0/((material.x/(2*nSta))/(lambda[i]*A) + (material.x/(2*nSta))/(
      lambda[i - 1]*A));
  end for;
